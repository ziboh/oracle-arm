...
def create_app(
    ...
    def oci_resources():
        try:
            ...
            return jsonify(error="OCI 配置无效：{}".format(exc)), 400
        except oci.exceptions.ServiceError as exc:
            return jsonify(error="OCI 请求失败：{} ({})".format(exc.message, exc.status)), 400

    @app.post("/api/oci/configure")
    def configure_oci():
        private_key = request.files.get("private_key")
        if private_key is None or not private_key.filename:
            return jsonify(error="请选择创建 OCI API Key 时下载的 PEM 私钥"), 400
        try:
    ...
    def test_notification():
        ...
        if enabled_field is None:
            return jsonify(error="通知渠道不受支持"), 400

        form = request.form.copy()
        for field in ("telegram_enabled", "bark_enabled", "webhook_enabled", "email_enabled"):
            form.pop(field, None)
        ...
        message = "A1 Control 通知测试：如果你收到这条消息，说明当前渠道配置有效。"
        outcomes = app.notification_sender(settings, message, emit=lambda _: None)
        outcome = outcomes[0] if outcomes else None
        if not outcome or not outcome["ok"]:
            detail = outcome["detail"] if outcome else "未发送"
            return jsonify(error="测试通知发送失败：{}".format(detail)), 502
    ...
    def change_password():
        ...
        if len(new_password) < 8:
            return jsonify(error="新密码至少需要 8 个字符"), 400
